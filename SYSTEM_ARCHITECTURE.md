...
**Daily bar archive (the only forward data capture).** `scripts/daily_bars_archive.py:run` stores split-adjusted Alpaca daily bars for 30 ETFs in `data/daily_bars.db` at 22:15 UTC on weekdays. Nothing in the trade path reads it. It exists because after the Unusual Whales exit the estate archived nothing at all, and the two strategies the research has cleared - the RSI(2) ETF dip-buy and the 200-day switch - are priced on daily bars (docs/feature_map/daily-bars.md).
