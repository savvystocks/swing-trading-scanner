...
def run_catalyst_scan(target_date=None, top_pct_strong=5, top_pct_watch=15,
    ...
    try:
        ...
        try:
            from src.catalyst.earnings_history import enrich_picks_with_earnings_history
            enrich_picks_with_earnings_history(ranked_picks, max_picks=20, verbose=verbose)
        except Exception as e:
            if verbose:
        ...
        try:
            ...
            enrich_picks_with_gex(ranked_picks, max_picks=15, verbose=verbose)
        except Exception as e:
            if verbose:
                print(f"  dealer_gex enrichment failed (non-fatal): {type(e).__name__}: {e}")

        try:
            from src.catalyst.options_positioning import enrich_picks_with_options_positioning
            enrich_picks_with_options_positioning(ranked_picks, verbose=verbose)
        except Exception as e:
            if verbose:
