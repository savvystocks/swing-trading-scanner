...
| 18 | Strategy search for a second edge (owner order 2026-09-18: "throw the kitchen sink at it") | D | REJECTED 2026-09-19 | A configuration had to clear a pre-registered bar AND an independent attack AND a held-out six months: six families, 13,841 configs, twelve passes, zero survivors, holdout unspent - reports/research/kitchen_sink_2026-09-19.md |
...
| 46 | Money | 2026-09-20: XSP credit spread seated on the Proof account without the court (4/8 HOLD), and the $1,000 per-trade cap lifted for that book so it trades the discovery 2%/4% width — worst case about $1,490, roughly 30% of the $5,000 account | NORTH_STAR v1.9 + proof_account.seats |
| 47 | Scope | 2026-09-21: credit spread only. Every directional strategy (the 14 flow probes, the student, the share probes, the put-debit spread) is switched off and closed as REJECTED - five months, four independent tests, no edge. Unusual Whales ended; the credit spread needs none of it | NORTH_STAR v1.9 + uw_scanner.enabled=false |
...
| 36 | Strategy | 2026-09-10/11: the STUDENT on the roster. Superseded basis: the shadow-ledger +10/day was k-fold flattered and the first two formula searches were leak-driven (first-print entry; entry-day regime close). LANDED 2026-09-11 in SHADOW mode: one ranked STUDENT seat, six pickers (A-F) trained on honest as-of features (15), dated dependency-free models, walk-forward thresholds, passive score log; walk-forward AUC ~0.51 (C pulled at 0.487). Flip to live only on the owner's five panel decisions (court unit, how many live, Monday path, auto-pull rule, one-seat design) | probe.student + sandbox STUDENT seat |
| 45 | Strategy | 2026-09-17 23:40 to 2026-09-19: ONE STRATEGY UNDER EVIDENCE, AND THE SEARCH IS CLOSED (owner 09-17: "going forward we are going to do 1 strategy and focus and collect data on that as we havent had a single strategy to ever find an edge"; owner 09-18: "throw the kitchen sink at it and don't come back until you create a strategy that is profitable and reliable"). The search is item 18, REJECTED: nothing in six families survives a pre-registered bar plus an adversarial rebuild, and at $5,000 / five slots one ordinary bad week of any near-miss is 50-99% of the account. CREDIT_SPREAD_W is the only structure with a live positive record and the only one evidence is being gathered FOR. Also REJECTED on measurement this week, not to be re-proposed without new data AND a new mechanism: the directional seats rebuilt as debit spreads (the +4.88 pts/day was a stop test skipped on days the short leg had no quote; on the court engine the spread loses monotonically in the size of its credit); a $1,600 directional slot (best cell t 2.69 against a 2.90 permutation bar, second half negative); a 50% take-profit on the credit spread (paired t 1.25, and -$4.56/wk on the weeks the BEAR gate actually trades). MEASUREMENT RULES now standing: a change to a live rule is reported as the PAIRED difference against that rule, never two level statistics side by side; leave-one-period-out on the face of every finding; a study of a live book applies that book's own gates before scoring; every premium-selling result carries the Clopper-Pearson tail bound at realised AND maximum loss; any study on the option archive refills weeks dropped for a missing strike at the most candidate-favourable bound. NOT decided by this row: the six directional seats still run as paper probes - pulling them is a separate owner decision. | reports/research/kitchen_sink_2026-09-19.md; BREAKDOWNS 2026-09-17 to 09-19; scripts/xsp_quote_log.py |
| 44 | Ops | 2026-09-15 22:55 (owner, on the Tuesday department review): FIX THE STUCK CLOSES AND SCOPE THE CONTRACT CAP. Three positions (ON expiring 09-18, QQQ, SPY) could not be closed because a stop the record called retired was still resting and reserved the contract; the exit path now reads the BROKER for resting sells before any close and a reserved contract is alerted once instead of looping silently. The subordinated max_contracts_per_ticker now counts only a named probe's OWN contracts - it had refused STUDENT_A's first live pick and WINNER_PROFILE on XLE the day after the held-name rule was loosened for the same reason. The three stale stops were cancelled by hand the same night. | BREAKDOWNS 2026-09-15 (both entries); MOT 6.26 |
| 43 | Ops | 2026-09-15 02:05: THE FADE STAND-DOWN HOIST (owner: "hoist its shape check before the sweep"). The regime-router term of fade_book.direction is candidate-independent (BULL and MILD stand the fade book down), so it is checked before the sensor sweep (fade_book.stood_down); the decision is unchanged, the reason text and harvest skip code are the same, BEAR and unknown regimes fall through to the full check. Saves ~5.5 sweeps per cycle on non-bear days. Retiring the fade book's entries was offered and NOT chosen; it keeps its evidence chain. | MOT 6.25; drill 9; entry-path.md |
| 42 | Ops | 2026-09-15 01:45: DEAD WEIGHT IN THE FILTER CHAIN (owner: "1 and 2" - retire the fade reservation and inert keys; study the scan pre-filter). SHIPPED: the generic probe pool is the top 12 scan names; the fade book no longer reserves the two biggest-flow names per cycle (MOT 6.24). NOT removed, with reasons: entry.max_spy_dist_pct (99, disabled by value) is a tuner-wired LIVE key (sunday_boundary steps/LIVE_WIRED); probe.early_strength feeds the early_strength watcher (live); probe.momentum is read by momentum_probe (disabled, code retained). STUDY (reports/research/scan_prefilter_2026-09-15.md): admitting names by the full $0.30-9.90 band adds a median 2 names/day that lose -10.8/day (t -2.7); the cheap band is the pool's worst slice (-14.5/day, t -10.9) and it is the control's universe. Verdict: do not widen; recorded, no change. FOUND, decision pending: the fade book's shape check runs AFTER the full sensor sweep, so it sweeps 5.5 unheld names per cycle (170 over 31 cycles on 09-14) for a book with no entry since 2026-08-20 and a tuner starved on throughput; options are to retire its live entries, or hoist the shape check before the sweep (behaviour-preserving). | probe-roster.md; MOT 6.24 |
| 41 | Strategy | 2026-09-15 00:50: THE PROBE FUNNEL (owner: "loosen the parameters so it actually trades"). Diagnosis in reports/research/probe_funnel_2026-09-15.md: the probes are starved by the $1,000 price band, not by their conditions - every cell is flat or negative on its own return below $9.90 and positive above $16; loosening regime or dip conditions adds fills that lose. RULING: the $1,000 slot STAYS (a $1,600/$2,500 amendment was offered with the numbers and declined for now); ship only what the archive supports - spread cap 2% -> 3%, DIP_CONVEXITY on SPY-below-50d (live cell, wide exit: own +8.75/day t 3.72 on 103 days), probes no longer blocked by other strategies' older positions on a name (same strategy, same-day and pending orders still block; occ_collision keeps one record per contract). The court does not demote for silence. | NORTH_STAR cap; probe-roster.md; MOT 6.23; drill 8 |
| 40 | Ops | 2026-09-13 23:50: THE RETURNS MAP (owner: "do this but for performance on returns"). Every performance number lives in one ledger recomputed from the book (the court's construction) and the v3 corpus; the performance map quotes only ledger tokens; a lint in the gate (MOT 6.19) fails on a stale or mismatched number; nightly alarms page once per condition (big trade, live-vs-archive divergence, jackpot concentration, capture under 0.6). Built from the 2026-09-11 workshop's pattern (feature map + verification + hard rules) after the engine feature map (MOT 6.18), the one-command verification, the page bundle and the reviewer briefing shipped the same night. | docs/performance_map; scripts/returns_ledger.py; scripts/performance_map_lint.py; scripts/returns_alarms.py; reports/performance/ledger.md |
| 39 | Strategy | 2026-09-13 22:35: STUDENT_A LIVE on the cheap-pick slice (owner's word; executed-slice export +31.0%/trade, weekly t 1.64 over 35 weeks, not pulled). 22:45: the EXIT SEARCH runs on A's LIVE picks once it has 20 closed fills - scripts/student_live_exit.py, Saturday 09:00 UTC, 210-config grid replayed on the fills' hourly paths, leave-one-out best vs BASE, report only; an exit change would go to the tuner's anchor set, never straight live. | reports/research/student_live_exit_<date>.md |
| 38 | Strategy | 2026-09-12: THE STUDENT CHEAP-PICK PATH. The spread variant of picker A is REJECTED (stale-print-guarded study: where a second leg could be priced the spread turned +6.5%/trade into -16.6%; deep-in-the-money picks cannot become $1,000 spreads). The "$1,600 student seat" amendment is WITHDRAWN (none of A's expensive holdout picks cost under $16). Owner ruling: the seat scores the ARCHIVE universe at contract level (both sides, premium 50k-1M, ask-side, spread <= 2%, ask >= 0.30 with no ceiling, DTE >= 1, top 30 by premium) and executes only picks whose live ask is under the $1,000 cap, at engine sizing (lots = $1,000 / cost); an unaffordable pick spends one of the picker's three weekly units, once per contract-day, as the study did. A LIVE on that slice: search +24.7%/trade t 1.19, holdout +52.1% t 1.46, ~1.5 trades a week - suggestive, not court-grade; the court on trading days decides. B/D/E/F shadow, C pulled. Cap unchanged. The pull rule judges the EXECUTED slice. Superseded: the seat's $4-9 call pool and A's AFFORD threshold cohort (BREAKDOWNS 2026-09-12 fourth entry). | reports/research/student_spread_2026-09-12.md; reports/student_cheap_pick_design_2026-09-12.md; MOT 6.17; drill scenario 7 |
| 37 | Strategy | 2026-09-12: STUDENT_FAMILY is judged in the Friday court on trading DAYS - 8 shared days vs the control, the same bar as the daily probes (weeks had been a cadence call, not a ruling). Nothing is on the clock until a picker is live under the $1,000 cap. | scripts/sunday_boundary.py + MOT 6.11 |
...
- **XSP quote-width verdict - about 2026-10-17.** Every backtest of CREDIT_SPREAD_W priced SPY; the book trades XSP (same price
  level and notional, unknown quote width). `scripts/xsp_quote_log.py` will have logged about forty paired XSP/SPY snapshots of the
  legs the book would trade. The rule is fixed in its docstring: median entry-window excess friction per spread, weeks as the unit of
  evidence. Accept: <= $5 and the SPY backtests transfer; $5-20 and the ~$20-26/wk expectation is haircut by that much; >= $20 and
  XSP's own spread consumes the edge and the book is re-examined. The two cron lines come out once the verdict is read.
- **Pooled weekend-effect test - owner's word, and only after the archive re-pull completes (about 2026-09-28).** The one mechanism
  the 2026-09-18 search found: index put spreads held Friday close to Monday close are breached one half to one sixth as often as
  Thursday to Friday at the same credit (18 of 18 cells, 2024-26). It will be pre-registered as six pooled cells with a ~2.5 bar and
  run ONCE on the untouched holdout (2026-03-16 .. 2026-09-15), on the complete chain, at XSP economics. Accept: clears the bar on
