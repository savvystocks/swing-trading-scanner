...
def main():
    ...
    seen = set()
    for band, pmin, pmax, amin, amax in (("whalepool_capped", 400000, 1000000, 0.30, 4.00),
                                         ("whale_anyask", 400000, 1000000, 0.0, 1e9),
                                         ("inband_expensive", 50000, 400000, 4.00, 1e9),
                                         ("liveband", 50000, 400000, 0.30, 4.00)):
        seen.clear()
    ...
    for band in ("whalepool_capped", "whale_anyask", "inband_expensive", "liveband"):
    ...
    open("reports/research/mild_conf_split_2026-09-01.md", "w", encoding="utf-8").write("\n".join(L) + "\n")
    print("\n".join(L), flush=True)
